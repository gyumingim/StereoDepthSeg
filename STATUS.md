...
**오프셋 실측(2026-09-22)**: `TotalCaptureResult.getPhysicalCameraResults()` 의 물리별 `SENSOR_TIMESTAMP` 를 읽으니(앱 `timing` 콜백,
센서 패킷 타입 100 `frame_timing`) **초광각(2)이 광각(5)보다 56~89 ms 먼저 노출**, 광각 = 논리 timestamp. 두 센서가 각자 66.666 ms
주기로 free-run 하며 위상이 초당 ~4 ms 흘러 약 16 초 주기로 한 바퀴 돈다. 노출 16.7~33 ms, 롤링셔터 skew 8.7 ms.
즉 "같은 timestamp" 쌍은 실제로는 평균 ~75 ms 차이였다 (20 cm/s × 75 ms = 15 mm = 0.5 m 에서 19 px, 관측 3~30 px 와 일치).
대응:
- **앱이 프레임 timestamp 를 물리 노출 시각으로 보정해 보냄**(논리 ts + 마지막 물리 오프셋; 오프셋 미확인 첫 프레임은 안 보냄, `--raw-ts` 로 예전 동작).
  PC 는 `--max-skew-ms 35`(기본) 로 실제로 가장 가까운 프레임끼리 묶는다 → 20 초 292쌍에서 **좌우 노출차 중앙값 5.5~6.0 ms, p95 14 ms**
  (보정 전 ~75 ms). 보정 후 드물게 뒤로 가는 timestamp(HAL 재짝짓기)는 치명 오류 대신 `reordered_dropped` 로 버린다.
- **잔차 동안의 폰 회전은 자이로로 보상**: 두 노출 시각 사이 자이로 창(`gyro_window`)을 적분해 그 쌍의 R 을 R·Rg 로 바꿔 정류
  (`lib_phone_depth.skew_rotation`, 0.02° 넘을 때만 테이블 재생성 ~10 ms). 물체 자체의 움직임은 보상 불가.
...
남는 한계: 잔차 6 ms(p95 14 ms) 동안 물체가 0.5 m/s 로 움직이면 3~7 mm = 1 m 에서 시차 1.4~3 px 오차(참 시차 7 px) → 움직이는 물체는
여전히 20~40 % 틀릴 수 있다(보정 전 200 %+). 하드웨어 동기(`CALIBRATED`)는 벤더 HAL 속성이라 앱에서 켤 수 없다 — 웹 조사 결과는 아래
"하드웨어 동기 조사" 절.
