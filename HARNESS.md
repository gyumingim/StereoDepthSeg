# 현재 카메라 작업 범위 (2026-09-22 사용자 승인)

- 사용자 지시: 스마트폰 후면 2개를 먼저 PC로 동시 전송하고 PC에서 추론. 3개는 지원되면 추가 시도. 이 범위의 코드 수정·새 코드 작성·실기기 시도가 명시적으로 승인됐다.
- 구현: `phone_stereo/`, `lib_phone_stereo.py`, `lib_phone_depth.py`, `z_phone_stereo.py`. 실행·검증과 현재 제한은 `phone_stereo/README.md`, `STATUS.md` 참조.
- 영상 표시·YOLO 성공과 FFS의 미터 거리 검증을 구분한다. 폰 렌즈 쌍에 노트북/폰의 기존 보정값을 재사용하지 않는다.
